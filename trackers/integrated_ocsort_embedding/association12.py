...
def associate(
    ...
    if not emb_off:
        ...
        for i in sorted_keys:
            ...
            single_cosine_cost =  None if (trk_embs.shape[0] == 0 or det_embs.shape[0] == 0) else det_embs @ np.array(memory[i]).T
            arr = np.exp(np.abs((det_embs - np.array(memory[i]).mean(axis=0)).sum(axis=-1)))
            arr_normalized =1- ( np.interp(arr, (arr.min(), arr.max()), (0, 1)))
            # import pdb;pdb.set_trace()
            if(single_cosine_cost is not None):
                memory_emb_cost.append(single_cosine_cost.mean(axis= -1))
         
        memory_emb_cost = np.array(memory_emb_cost).T
    if not emb_off:
        long_memory_emb_cost = []
        sorted_keys = sorted(long_memory.keys())
        for i in sorted_keys:
            
            single_cosine_cost =  None if (trk_embs.shape[0] == 0 or det_embs.shape[0] == 0) else det_embs @ np.array(long_memory[i]).T
            arr = np.exp(np.abs((det_embs - np.array(long_memory[i]).mean(axis=0)).sum(axis=-1)))
            arr_normalized =1- ( np.interp(arr, (arr.min(), arr.max()), (0, 1)))
            # import pdb;pdb.set_trace()
            if(single_cosine_cost is not None):
                long_memory_emb_cost.append(single_cosine_cost.mean(axis= -1)*arr_normalized)
        
        long_memory_emb_cost = np.array(long_memory_emb_cost).T
        # import pdb;pdb.set_trace()
    ...
    if min(iou_matrix.shape) > 0:
        ...
        if a.sum(1).max() == 1 and a.sum(0).max() == 1:
            ...
            if(emb_cost.shape == memory_emb_cost.shape):
                x =  (1- (np.abs((iou_matrix- emb_cost))**(0.5)))*0.3
                # import pdb;pdb.set_trace()
                # emb_cost +=  (long_memory_emb_cost + memory_emb_cost)*0.5
                emb_cost += (memory_emb_cost*w_assoc_emb + long_memory_emb_cost*w_assoc_emb)*0.5
